...
@Component({
  ...
  styles: [`
    ...
    .all-videos-btn .btn-icon {
      font-size: 1rem;
    }

    .all-videos-btn .btn-content {
      flex: 1;
      display: flex;
      flex-direction: column;
    }

    .all-videos-btn .btn-title {
      font-size: 0.75rem;
      font-weight: 600;
      color: white;
    }

    .all-videos-btn .btn-count {
      font-size: 0.5625rem;
      color: rgba(255, 255, 255, 0.7);
    }

    .all-videos-btn .btn-arrow {
      font-size: 1rem;
      color: rgba(255, 255, 255, 0.7);
    }
    ...
    .video-card.playing .playing-indicator {
      color: #22c55e;
    }
...
export class RemotePreviewComponent implements OnChanges {
  ...
  // Cached computed values to avoid creating new arrays on each change detection
  ...
  private rebuildCaches(): void {
    ...
    // Rebuild categories for time cache
  ...
  private computeAllVideos(): VideoItem[] {
    ...
    // From local videos (not already in config)
  ...
  private computeCategoriesForTime(timeId: string): CategoryConfig[] {
    ...
    // Fallback: return all categories for demo
    if (timeId === 'before') {
  ...
  selectPhase(phase: string): void {
    ...
    this.playingVideoPath = null; // Reset playing video when phase changes
  }
  ...
  get currentLoopVideoCount(): number {
    // For 'neutral' phase, use global sponsors/loop
    if (this.selectedPhase === 'neutral') {
      return this.config?.sponsors?.length ?? 0;
    }

    // Find the timeCategory matching the selected phase
  ...
  simulatePlay(video: VideoItem): void {
    // Toggle play state for demo purposes
  ...
  getRecentVideos(): VideoItem[] {
    return this.cachedRecentVideos;
  }
}
