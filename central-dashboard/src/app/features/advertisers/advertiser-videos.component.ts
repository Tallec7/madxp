...
export class SponsorVideosComponent implements OnInit {
  ...
  async addSelectedVideos() {
    ...
    try {
      ...
      this.notification.error(err.message || 'Erreur lors de l\'ajout');
    } finally {
      this.adding = false;
    }
  }
  ...
  async savePriority() {
    ...
    try {
      ...
      this.notification.error(err.message || 'Erreur lors de la sauvegarde');
    } finally {
  ...
  async saveOrder() {
    try {
      ...
      this.notification.error(err.message || 'Erreur lors de la réorganisation');
