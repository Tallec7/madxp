...
start_chromium() {
    ...
    (
        ...
        if [[ -n "$wid" ]]; then
            ...
            # 5. Re-raise de sécurité : LXDE/openbox peut restacker lxpanel au-dessus
            # de Chromium 1-5s après le premier fullscreen (session init, network events).
            # Sans ce re-raise, la barre de tâches reste visible ~30s (jusqu'au prochain
            # check_window_stacking dans le watchdog loop).
            # Délais incrémentaux : sleep 3 → recheck à ~+3s, sleep 5 → ~+8s, sleep 7 → ~+15s
            for _sleep_delta in 3 5 7; do
                sleep "$_sleep_delta"
                if ! kill -0 "$CHROMIUM_PID" 2>/dev/null; then break; fi
                _active=$(DISPLAY=:0 xdotool getactivewindow getwindowname 2>/dev/null || true)
                if echo "$_active" | grep -qi "panel"; then
                    log "⚠️ BOOT RESTACK: lxpanel repassé devant — re-raise"
                fi
                # Toujours ré-appliquer (idempotent) — rattrape aussi les décalages subtils
                DISPLAY=:0 xprop -id "$wid" -f _MOTIF_WM_HINTS 32c -set _MOTIF_WM_HINTS "0x2, 0x0, 0x0, 0x0, 0x0" 2>/dev/null
                DISPLAY=:0 xdotool windowmove "$wid" 0 0 2>/dev/null
                DISPLAY=:0 xdotool windowsize "$wid" "${PRIMARY_SCREEN_WIDTH:-$DEFAULT_SCREEN_WIDTH}" "${PRIMARY_SCREEN_HEIGHT:-$DEFAULT_SCREEN_HEIGHT}" 2>/dev/null
                DISPLAY=:0 xdotool windowactivate "$wid" 2>/dev/null
            done
            log "✓ Séquence re-raise boot terminée"
        else
