...
perform_single_scan() {
    ...
    # Write inter-process scan cache so that networkDetector (Node.js)
    # can reuse our results instead of launching a SECOND iwlist scan.
    # RTL8192EU single-radio cannot tolerate 2 scans within 120s.
    if [ "$SCAN_INTERFACE" = "wlan1" ]; then
        ...
        log "Scan cache written to /tmp/neopro-wlan1-scan-cache"
    fi
