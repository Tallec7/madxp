...
socket.emit('heartbeat', {
  ...
  kioskStatus: {            // optionnel — écrit par kiosk-watchdog.sh
    ...
    gpuDecodeMode: 'hardware',        // (v3.99+) 'hardware' = V4L2 stateless (Pi 5), 'software' = fallback
                                      // 'software' déclenche alerte fleet gpu_decode_fallback + Prometheus counter
  },
...
| `site_type` | Stratégie ADR-069 (via `deliveryStrategyRegistry`)                                               |
| ----------- | ------------------------------------------------------------------------------------------------ |
| `'pi'`      | `PiSocketStrategy` → `sendOrQueue(deploy_video)` → Pi télécharge, confirme via `deploy_progress` |
| `'saas'`    | `SaasDirectStrategy` → marque `completed` immédiatement (vidéos servies via URL FTP, pas de Pi)  |
| `'demo'`    | Traité comme `pi` par `PiSocketStrategy.canHandle` (siteType !== 'saas')                         |

> **Monitoring :** `checkStuckDeployments()` dans `alerting-checks.service.ts` exclut les sites SaaS des alertes "Déploiement bloqué" via `JOIN sites WHERE site_type != 'saas'`. Les déploiements Pi bloqu��s >30min génèrent une alerte warning, >60min critical, et les OTA >2h sont auto-failed.
...
>
> **Monitoring** : `checkEmptySaasProfiles()` dans `alerting-checks.service.ts` détecte les profils SaaS vides toutes les 5 minutes. Smoke test enforced.
...
**Fallback catégorie (v3.115.2) :** Quand `deployedPathsMap` n'a pas de chemin réel (site hors ligne, déploiement en attente), le dashboard construit un chemin spéculatif `videos/${category || 'default'}/${filename}`. Le fallback `'default'` est aligné avec `deployment.service.ts` (ligne ~361) qui envoie `category: 'default'` au Pi. **Important :** ne jamais utiliser `'UPLOADS'` comme fallback — cela crée un mismatch avec le Pi qui ne connait pas cette catégorie (smoke test enforced)
...
| **Deployment**      | `deployment.service.ts` + `services/delivery/` (ADR-069)                                                    | Orchestration déploiement vidéos — délègue à `deliveryStrategyRegistry` (PiSocketStrategy / SaasDirectStrategy)                                                                                                                                                                                                                                                                    |
...
| **Alerting**        | `alerting.service.ts` + `alerting.types.ts` + `alerting-notifier.service.ts` + `alerting-checks.service.ts` | Alertes multi-canal (email, slack, webhook) — 22 seuils par défaut. Core CRUD + évaluation dans `alerting.service.ts`, types/thresholds dans `alerting.types.ts`, notifications dans `alerting-notifier.service.ts`, vérifications périodiques (stuck deployments, phantom sponsors, aggregation staleness, empty SaaS profiles, hourly metrics) dans `alerting-checks.service.ts` |
...
>
> **Monitoring agrégation :** `alerting-checks.service.ts` vérifie toutes les 5 min que `club_daily_stats` et `site_sponsor_daily_stats` ont été agrégées dans les 36 dernières heures. Si le CRON ne tourne pas, alerte `aggregation_stale` critique — sans agrégation, les données sont perdues après le cleanup `video_plays` (15j).
...
Le site-detail est organisé en **6 onglets** avec des composants Angular standalone. L'affichage est **conditionnel** selon le `site_type` : les sites SaaS ont un onglet État simplifié et l'onglet Debug est masqué.

#### Vue conditionnelle SaaS vs Pi

Le getter `isSaas` (`site?.site_type === 'saas'`) dans `site-detail.component.ts` pilote l'affichage :
...
| -------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **État**       | `site-detail.component.ts` | **Pi** : Métriques hardware, connexion temps réel, alertes, ventilateur. **SaaS** : URL d'accès, activité temps réel, performance contenu 7j                                                                                                                                                                                                                                                                                                                  |
| **Contenu**    | `SiteContentTabComponent`  | Coordinateur fin : charge les données, orchestre 4 sous-composants. Sous-composants : `VideoManagerComponent` (upload, bibliothèque, suppression), `ConfigEditorComponent` (catégories, boucles, télécommande, analytics), `DeploymentStatusComponent` (déploiement, diff, validation), `ConfigDraftComponent` (brouillons, historique)                                                                                                                       |
...
| **Debug**      | `SiteDebugTabComponent`    | **Pi uniquement** (masqué pour SaaS). Coordinateur fin + diagnostic guidé. Sous-composants : `HealthMonitorComponent` (santé GPU/fan/display/services/diagnostics), `CommandPanelComponent` (terminal, logs, export bundle, connexion zombie), `ServiceStatusComponent` (réseau, buffer, hotspot, WiFi BSSID/client), `SystemInfoComponent` (fichiers Pi, historique config, timeline), `DebugSummaryBarComponent` (barre résumé), `pollCommand<T>()` utility |
...
| **GPU decode mode (Pi 5)** | V3.99+ : hardware decode via V4L2 stateless API (BCM2712 H.264 decoder). Chromium feature flag `V4L2FlatVideoDecoder` active le décodage hardware → ~20% CPU en moins → réduit le coil whine PMIC. Auto-fallback : si Chromium crashe 2× avec hardware decode, bascule en software pour le reste du boot. Fichier `/tmp/gpu-decode-fallback` (tmpfs, effacé au reboot → re-tente hardware). `kiosk-status.json` : `gpuDecodeMode` ("hardware" / "software"). Heartbeat → alerte fleet `gpu_decode_fallback` + Prometheus `neopro_gpu_decode_fallback_total`. Health report : -5 points si software. Smoke tests: 11 guards                                            |
...
>
> ⚠️ **Ne jamais dupliquer `--enable-features` dans les arrays common_flags/gpu_flags** — même règle que `--disable-features` : Chromium n'honore que le dernier flag. Combiner dans la variable `$enable_features`. Smoke test enforced (v3.99+).
>
> ⚠️ **Ne jamais mettre `--disable-gpu-memory-buffer-video-frames` dans le bloc hardware decode Pi 5** — ce flag force le chemin software complet, rendant V4L2FlatVideoDecoder ineffectif. Réservé au bloc software fallback uniquement. Smoke test enforced (v3.99+).
