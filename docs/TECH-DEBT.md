...
- **Coût aujourd'hui** : 12 références mortes dans `config_profiles` (NLF + Demo SaaS), 27 entries dans `localSponsors[]` (NLF: 14, Bottière: 13), faux positif `aggregation_stale critical` chronique sur clubs inactifs >36h (corrigé PR [#894](https://github.com/Tallec7/madxp/pull/894), commit `d0754257`), 2306 plays NLF perdus pour les analytics par sponsor (cleanup data appliqué prod 2026-05-07).
- **Effort** : 3 phases issues GitHub. Phase 1 cloud ([#891](https://github.com/Tallec7/madxp/issues/891) — FK + validation API + cleanup migration + ADR léger, ~1j) — débloque les autres. Phase 2 Pi OTA ([#892](https://github.com/Tallec7/madxp/issues/892) — re-réconciliation localSponsors, ~1.5j). Phase 3 observability ([#893](https://github.com/Tallec7/madxp/issues/893) — métrique + Grafana, ~0.5j si bundle).
