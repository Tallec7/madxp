...
| 7 (V2) | (cette PR)                                         | Hardening V2 : seed script `seed-white-glove-template.ts` (démo white-glove idempotente), +7 smoke tests Sprint 6 (Premium gate + filtre UI + migration + seed), E2E Playwright `template-scope-filter` (défensif seed-optionnel) | 🟠 WIP   |
