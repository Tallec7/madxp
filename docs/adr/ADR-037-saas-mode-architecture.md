...
| Remote tab comptée comme écran (clientType 'saas-tv')   | `emitSaasRegisterIfNeeded()` détecte `/remote` → envoie `'saas-remote'` ; `getSaasClientCount()` itère la room et ne compte que `clientType === 'saas-tv'` — smoke test enforced (v3.136.2)                                        |
| SaaS analytics silencieuses (recording OFF au boot)     | `tv.component.ts` appelle `startRecording(false)` + `startSession()` au boot quand `saasMode && !isSlaveMode && !secondary` — smoke test enforced (v3.136.3)                                                                       |
| Vidéos SaaS sans métadonnées sponsors (video_id absent) | `getSaasConfig` et `getSaasProfileConfig` appellent `enrichConfigWithAnalyticsMetadata()` avant `resolveVideoUrls()` — le spread `{...v}` préserve les champs injectés — warning log si 0 enrichi — smoke test enforced (v3.136.3) |
