...
| ------ | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- |
| 1      | [#493](https://github.com/Tallec7/madxp/pull/493) | Migration `template_variants` / `_layers` / `_text_fields` / `_image_slots`, API super_admin CRUD, runtime data-driven, fallback v1                                                                                                                                                                                                                                                                                                                                                                                | ✅ Mergé |
| 2      | [#501](https://github.com/Tallec7/madxp/pull/501) | Studio user mode (Angular) : `StudioV2EditorComponent`, preview React-in-Angular via `@remotion/player`, upload user images (ADR-077)                                                                                                                                                                                                                                                                                                                                                                              | ✅ Mergé |
| 3      | [#504](https://github.com/Tallec7/madxp/pull/504) | Studio super_admin mode : `AdminStudioPanelComponent` (variants / layers / fields), `CreateTemplateWizardComponent`, CRUD admin wiré au dashboard, E2E Playwright                                                                                                                                                                                                                                                                                                                                                  | ✅ Mergé |
| 4      | [#506](https://github.com/Tallec7/madxp/pull/506) | Migration shadow-seed ButSimple / ButImgJoueur (idempotente, `schema_version=1` préservé), tests permissions `super_admin` sur 8 routes × 5 rôles, smoke guards                                                                                                                                                                                                                                                                                                                                                    | ✅ Mergé |
...
| 6 (V2) | [#514](https://github.com/Tallec7/madxp/pull/514) | Gallery filter "Mes templates perso" + badge Club (dashboard), feature gate `template_studio_club_scoped` (Premium) côté front/back, render-time guard site_id + tier, migration `add-template-studio-v2-club-scoping.sql`                                                                                                                                                                                                                                                                                         | ✅ Mergé |
| 7 (V2) | [#515](https://github.com/Tallec7/madxp/pull/515) | Hardening V2 : seed script `seed-white-glove-template.ts` (démo white-glove idempotente), +7 smoke tests Sprint 6 (Premium gate + filtre UI + migration + seed), E2E Playwright `template-scope-filter` (défensif seed-optionnel)                                                                                                                                                                                                                                                                                  | ✅ Mergé |
...
- PR #537 : backfill one-shot des assets V2 pour les deux templates legacy multi-fragments (`ButSimple` 3 fragments, `ButImgJoueur` 5 fragments). Le seed `seed-but-simple-but-img-joueur-v2-shadow.sql` créait les variants/text-fields/image-slots mais laissait `background_video_url = ''` et zéro layer — v1 fallback sur `staticFile()` pour lire les `.webm` bundlés (templates-remotion/public/), v2 runtime data-driven exige des URLs http/blob/data → preview noir. Script `central-server/src/scripts/backfill-legacy-templates-v2-assets.ts` (npm `db:backfill-legacy-templates-v2`) upload les fragments vers FTP `template-assets/studio/legacy/`, seed `template_variants.background_video_url` (fragment A) et `template_layers[]` (fragments B..E, z-index incrémental). Idempotent : skip upload si URL déjà en DB, skip insert si layers déjà présents.
- PR #538 : le script TS de PR #537 s'est révélé non-exécutable depuis le laptop (DB Railway sur réseau privé interne, hostname `*.railway.internal` NXDOMAIN depuis l'extérieur). Retour au flow standard du projet : migration SQL idempotente `backfill-legacy-templates-v2-urls.sql` auto-appliquée au boot Railway via `node dist/scripts/migrate.js && node dist/server.js` (start script), trackée dans `schema_migrations`. URLs publiques hardcodées (`https://kalonpartners.bzh/neopro-video/template-assets/studio/legacy/BUT_*.webm`). Prérequis one-shot manuel : upload des 8 `.webm` via FileZilla avant merge. 3 smoke guards additionnels dans `smoke-remotion.test.ts` (migration existe + contenu + `isValidSrc` runtime guard). Le script TS de PR #537 reste en place comme fallback/doc mais n'est plus la voie canonique.
