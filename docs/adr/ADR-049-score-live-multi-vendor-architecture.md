...
Le système de score live MadXP est implémenté depuis déc 2025 en saisie manuelle via la Remote (Phase 1). C'est un **deal-breaker commercial** : plusieurs prospects amateurs refusent la double saisie (opérateur table de marque + opérateur MadXP) et exigent la lecture automatique depuis la table officielle.
...
Une contrainte terrain majeure s'impose : **80% des gymnases ciblés n'ont ni WiFi ni couverture 4G** dans l'enceinte, et MadXP a fait de l'**offline total** un pilier produit (ADR-024 network resilience). La solution doit fonctionner sans internet et sans WiFi club. Par ailleurs, la **table de marque est physiquement éloignée de l'écran** (~10m typiques), alors que le Pi MadXP est à côté de l'écran — tirer un câble RS-485 de 10m dans un gymnase est pénible à industrialiser.

Enfin, on a identifié en session une **opportunité produit majeure** : une fois F-15.2 livrée, MadXP deviendrait la seule entité capable de lire directement les tables de marque de centaines de clubs amateurs français. Exposer cette donnée comme API publique transformerait MadXP en **source of truth** du sport amateur, avec un effet réseau naturel et un nouveau flux de revenus.
...
- **Topologie B** — Gymnase offline. Scorebox déporté (Pi Zero 2 W + HAT RS-485 SN65HVD72) qui émet son propre mini-AP WiFi local. Le Pi principal s'y connecte via clé USB WiFi (`wlan1`). `wlan0` reste dédié au hotspot MadXP/Remote. Aucune dépendance internet ni WiFi club.

Le **MadXP Scorebox** est conçu comme un produit physique unique avec **trois modes configurables** :
...
- **wlan0 en mode mixte AP+STA concurrent** : rejeté car instable, non supporté par le driver `brcmfmac`, et incohérent avec ADR-011 (interdiction mesh). MadXP sépare wlan0 (AP) et wlan1 (STA) par principe.
...
- **Effet réseau long terme** : plus MadXP équipe de clubs, plus la data temps réel devient un asset monétisable et un argument d'acquisition
