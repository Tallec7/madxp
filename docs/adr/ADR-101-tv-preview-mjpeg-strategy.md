...
Adopter **MJPEG over HTTP en V1**, exposé par le admin-server Pi sur l'endpoint existant `:3001/preview.mjpeg`, consommé par la Remote via une simple balise `<img>` (multipart/x-mixed-replace géré nativement par les navigateurs). Pi 5 only en V1 (Pi 4 reste en placeholder, ré-évalué post-mesures prod).
...
- **MJPEG via mjpg-streamer (binaire C natif)** : rejeté pour dépendance lourde, packaging Debian/Raspbian additionnel, et perte du contrôle fin du throttle CPU/temp côté Node. Préférence pour `@julusian/jpeg-turbo` (libjpeg-turbo SIMD ARM NEON, ~600 KB) intégré au Express admin-server existant.
...
- **+** Time-to-market 4 j-h P0 + 3 j-h P1 = 7 j-h V1 livrable en 1 sprint. Pas de nouveau composant infra (pas de TURN, pas de SFU). Pas de migration DB. Pas de nouveau port (reuse :3001 admin-server, CSP nginx déjà ouverte).
...
- **−** CSP côté Remote : `img-src` à étendre à `http://*.local:3001` + `http://192.168.4.1:3001` (IP hotspot Pi) — pas de changement `script-src`. Documenté dans la SPEC + smoke test du wiring.
...
- **Auth obligatoire** : cookie `session-pi` admin-server (LAN) ou token HMAC TTL 5 min en query string (cloud distant exceptionnel).

## Fichiers impactés (P0 implementation)

- `raspberry/server/services/tv-preview.service.js` — nouveau (capture + encode + bus subscribe)
- `raspberry/admin/admin-server.js` — endpoint `GET /preview.mjpeg` monté
- `raspberry/server/services/socket-server.js` — events `tv-preview:capability/start/stop/throttled`
- `raspberry/sync-agent/src/services/hardware-detect.js` — détection Pi 5 vs Pi 4
- `raspberry/src/app/components/remote-v2/parts/r2-tv-monitor.component.ts` — `<img>` + onerror + reconnect backoff
- Remote V2 chassis — feature flag `previewEnabled` propagé via Socket.IO
- CSP nginx Remote — `img-src` étendu LAN
- `central-server/src/__tests__/smoke/smoke-tv-preview.test.ts` — wiring + throttle + single-subscriber
- `package.json` Pi (`@julusian/jpeg-turbo`, `puppeteer-core`)
