# 📊 Product Weekly — W14 · 2026-03-30 → 2026-04-05

> **Généré automatiquement le** : 18 avril 2026
> **Versions publiées** : v3.118.4 → v3.125.4 (45 releases)
> **PRs mergées** : 1 | **Commits directs** : 75
...
**dashboard** (17 commits)

- `25b5282` fix(dashboard): filter SaaS sites from OTA deployment selector
- `1e4ca74` fix(dashboard): include checksum in deploy_video from site-content-tab
- `148ffeb` fix(dashboard): add @bworlds/launchkit to root package.json for CI build
  _… +14 autres commits_

**club-portal** (13 commits)

- `12f8c21` docs(club-portal): sync documentation, add smoke tests and MADXP guard on updateVideo
- `63e2904` fix(club-portal): auto-tag uploaded_for_site_id for club users
- `a40c1c6` fix(club-portal): allow club users to delete/update their own videos
  _… +10 autres commits_

**templates** (10 commits)

- `2917223` feat(templates): browser-side video rendering (Canvas + MediaRecorder)
- `c95dec8` fix(templates): use PUPPETEER_EXECUTABLE_PATH env var on Railway
- `7d9f26a` fix(templates): remove /content prefix from API paths
  _… +7 autres commits_

**saas** (9 commits)

- `445571a` docs(saas): add defense-in-depth guard and smoke tests for OTA/SaaS exclusion
- `a3a6318` docs(saas): add smoke test and rule for connection indicator SaaS guard
- `b2c2679` fix(saas): hide connection indicator badges for SaaS sites
  _… +6 autres commits_

**ci** (9 commits)

- `3928c8f` fix(ci): deploy SaaS after dashboard to prevent clean-slate deletion
- `7e90d10` fix(ci): exclude /saas/ from dashboard .htaccess SPA catch-all
- `28ea6a0` fix(ci): exclude demo-configs from SaaS build assets
  _… +6 autres commits_

**server** (5 commits)

- `d621c32` fix(server): add findByIdFull mock to sponsor video sync test
- `4d40933` fix(server): fix 3 production DB errors spamming logs every ~30s
- `cefaa9f` fix(server): align Pi analytics routes on piAnalyticsRateLimit (500/min)
  _… +2 autres commits_

**sites** (3 commits)

- `befe97e` fix(sites): allow empty hardware_model in Joi validation
- `c7ddd4c` fix(sites): add site_type to Joi validation schema for create/update
- `acb3f44` refactor(sites): decompose 5 monolithic files into focused modules (-7020 lines)

**tests** (2 commits)

- `fd148eb` fix(tests): add site_type to test mock objects for SaaS compatibility
- `9f5078f` fix(tests): update upload-verification tests for checksum/file_size gate

**club** (2 commits)

- `a446052` fix(club): prevent club users from deleting MADXP corporate videos
- `45bf60e` fix(club): add uploaded_for_site_id to findVideoById SELECT query

**security** (2 commits)

- `7863288` fix(security): add missing Joi validateParams/validateQuery to 50+ routes
- `b7c1346` test(security): add 29 smoke tests for input validation coverage and SQL injection prevention

**safe-dashboard** (2 commits)

- `b58f510` fix(safe-dashboard): fix .dockerignore excluding markdown files + add watch patterns
- `0eb11c5` fix(safe-dashboard): include docs/safe/ in Docker image for SAFe parser

**lint** (1 commits)

- `1d7e187` fix(lint): remove unused HttpError class from sites.controller

---

## 6. DOCS & CHORES

**#435 — ci(dashboard): auto-deploy to Hostinger on release** ⬜

> - Adds a `deploy-dashboard` job to the release workflow — - Builds the Angular central-dashboard and uploads via FTP to `public_html/neopro-admin/`
...
| **1 PR(s)** sans section "Impact client" renseignée                              | Faible      | Faible (rapport incomplet, BO mal informé) | Remplir rétrospectivement ou accepter comme dette de processus            |
...
| **75 commits directs** sur `main` hors PR — revue de code partielle              | Modérée     | Moyen                                      | Passer par des PRs même courtes pour tracer les décisions                 |
...
2. [PROCESSUS] 1 PR(s) utilisent l'ancien template (pas d'"Impact client"). Remettre à jour ou accepter ?
...
4. [DISCIPLINE] 75 commits directs cette semaine. Renforcer la règle PR systématique ou assouplir formellement pour les chores ?
...
| #                      | Titre                                              | Type  | Mergé le   |
| ---------------------- | -------------------------------------------------- | ----- | ---------- |
| [#435](../../pull/435) | ci(dashboard): auto-deploy to Hostinger on release | chore | 2026-04-01 |
