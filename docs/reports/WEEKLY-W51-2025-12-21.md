# 📊 Product Weekly — W51 · 2025-12-15 → 2025-12-21

> **Généré automatiquement le** : 18 avril 2026
> **Versions publiées** : ? → ? (0 releases)
> **PRs mergées** : 4 | **Commits directs** : 0
...
**#274 — fix: improve error handling for software update creation** ⬜

> - Ajoute la création automatique du bucket `software-updates` dans Supabase Storage s'il n'existe pas — - Retourne une erreur spécifique (409 Conflict) si une version dupliquée est tentée

**#273 — fix: update sponsors array during video deployment for analytics tracking** ⬜

> - Les analytics des vidéos sponsors ne remontaient pas avec les `video_id` et `sponsor_id` car le tableau `configuration.sponsors` (utilisé par l'app TV) n'était jamais mis à jour lors du déploiement — - `deploy-video.js` : Ajout de la mise à jour automatique du tableau `sponsors` quand une vidéo sponsor est déployée (catégorie SPONSORS, analyticsCategory=sponsor, ou sponsorId défini)

**#272 — fix: dashboard health endpoint and static publishing** ✅

> - Fix the `/health` endpoint returning 404 on the dashboard service — - Update `staticPublishPath` to point to the `browser/` subdirectory where Angular 20 outputs build files

**#271 — fix: correct video deletion endpoint routing** ⬜

> - Fixed routing issue causing DELETE /api/videos/:id to return 404 — - Consolidated content routes middleware configuration
...
| **4 PR(s)** sans section "Impact client" renseignée                              | Faible      | Faible (rapport incomplet, BO mal informé) | Remplir rétrospectivement ou accepter comme dette de processus            |
...
2. [PROCESSUS] 4 PR(s) utilisent l'ancien template (pas d'"Impact client"). Remettre à jour ou accepter ?
...
| #                      | Titre                                                        | Type | Mergé le   |
| ---------------------- | ------------------------------------------------------------ | ---- | ---------- |
| [#274](../../pull/274) | fix: improve error handling for software update creation     | fix  | 2025-12-21 |
| [#273](../../pull/273) | fix: update sponsors array during video deployment for analy | fix  | 2025-12-21 |
| [#272](../../pull/272) | fix: dashboard health endpoint and static publishing         | fix  | 2025-12-20 |
| [#271](../../pull/271) | fix: correct video deletion endpoint routing                 | fix  | 2025-12-20 |
