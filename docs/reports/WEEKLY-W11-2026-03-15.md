# 📊 Product Weekly — W11 · 2026-03-09 → 2026-03-15

> **Généré automatiquement le** : 18 avril 2026
> **Versions publiées** : v3.101.2 → v3.107.1 (13 releases)
> **PRs mergées** : 0 | **Commits directs** : 16
...
- `c2dfd7e` feat(admin-ux): videos tab UX batch 2 — modal, search feedback, responsive, a11y
- `c79acc2` fix(admin-ux): improve videos tab usability — labels, density, mode awareness

**admin** (2 commits)

- `b5e3693` feat(admin): autonomie club — phases sponsor, récap, split vidéo, UX polish
- `7bf248f` fix(admin): remove git hash from generated app.js header

**video-library** (2 commits)

- `a605492` fix(video-library): scope stats bar to filtered videos instead of global catalog
- `1c3fcf6` feat(video-library): contextual config badges, advertiser column, duplicate detection, filename collision fix

**deploy** (1 commits)

- `8d9b801` fix(deploy): fix-fleet-pi.sh appelé sans sudo — 13 étapes fleet silencieusement ignorées

**local-content** (1 commits)

- `83833d9` fix(local-content): use correct column name for advertiser in getCloudVideos query

**pi5** (1 commits)

- `8a53290` fix(pi5): enable Active Cooler PWM fan control via dtparam=cooling_fan

**tv-component** (1 commits)

- `41c1e19` fix(tv-component): slave display freeze on manual video — LocalBroadcast + handleMasterLoopState guards

**deploy-backfill** (1 commits)

- `a54f8fd` feat(deploy-backfill): auto-heal deployed_path for pre-existing deployments

**data-pipeline** (1 commits)

- `94d607a` feat(data-pipeline): live VIEWs for same-day dashboard data + CRON aggregation

**deploy-feedback** (1 commits)

- `b14b281` feat(deploy-feedback): Pi reports real deployed video path back to central server

**dashboard** (1 commits)

- `b32d874` fix(dashboard): remove deprecated secondary display card and PC mode dead code

**sync-agent** (1 commits)

- `f748430` fix(sync-agent): orphan sponsor-impressions.js + stale buffer causing HTTP 400

**kiosk** (1 commits)

- `b915c33` feat(kiosk): GPU decode auto-fallback + hardware-only dual-display detection
