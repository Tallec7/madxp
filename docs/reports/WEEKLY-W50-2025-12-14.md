# 📊 Product Weekly — W50 · 2025-12-08 → 2025-12-14

> **Généré automatiquement le** : 18 avril 2026
> **Versions publiées** : ? → ? (0 releases)
> **PRs mergées** : 0 | **Commits directs** : 19
...
**config-editor** (10 commits)

- `a335203` fix(config-editor): fix categories display and analytics mapping
- `ce7e354` fix(config-editor): use setTimeout to force Angular change detection for categories
- `549f29e` fix(config-editor): show tabs during loading and add debug traces
  _… +7 autres commits_

**local-admin** (2 commits)

- `109b213` fix(local-admin): handle nullable form values in createClient
- `9e6ea6e` fix(local-admin): fix TypeScript error in clientForm definition

**dashboard** (2 commits)

- `b9774b6` fix(dashboard): handle paginated API response format for sites
- `75962a8` feat(dashboard): allow multi-video deployments

**server** (2 commits)

- `5469556` fix(server): start HTTP server immediately for Render health checks
- `14036b0` fix(server): allow DB CA files

**cors** (1 commits)

- `4210dd7` fix(cors): normalize origins and improve CORS debugging

**sync-agent** (1 commits)

- `4090511` feat(sync-agent): keep human friendly video names

**admin** (1 commits)

- `725b98e` fix(admin): allow sudo restarts from local UI
