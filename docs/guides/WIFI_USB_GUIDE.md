...
> **Note (v3.116.25+) :** Ce problème est aggravé par le reconfigure loop : quand le signal oscille autour du seuil de `_computeOptimalBgscan()`, chaque `wpa_cli reconfigure` coupe le carrier et fait basculer le seuil dans l'autre sens. La correction v3.116.25 ajoute une hystérésis (-67/-78 dBm) et skip le reconfigure si la config bgscan est identique.
...
│   └── (v3.116.26+) Fast-path : IP fix sans restart si hostapd/dnsmasq actifs
├── Internet Monitor (wlan1, toutes les 60s)
│   └── Vérifie : IP valide, ping gateway, ping 8.8.8.8
│   └── (v3.116.25+) Skip wpa_cli reconfigure si bgscan config identique
...
- **Toujours configurer bgscan** (`simple:30:-70:300`) — le seuil est ajusté dynamiquement par `autoOptimize()` avec hystérésis (-67/-78 dBm) depuis v3.116.25
...
| 22 mar | v3.116.25 | 15+ déconnexions/heure auto-infligées (NLF)                     | Bgscan reconfigure loop : signal -68/-73 dBm flip-flop le seuil `_computeOptimalBgscan()` → `wpa_cli reconfigure` toutes les 90s | Hystérésis -67/-78 dBm + skip reconfigure si config identique                                                                                                              |
| 22 mar | v3.116.26 | N autoOptimize parallèles à la reconnexion                      | `_networkProfileStarted` absent → chaque événement reconnexion relançait `autoOptimize()` en parallèle                           | Guard `_networkProfileStarted` dans agent.js                                                                                                                               |
