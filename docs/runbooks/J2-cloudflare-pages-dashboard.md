...
> **Objectif** : URL `neopro-exg.pages.dev` (auto-deploy sur push `main`) + PR previews automatiques. Hostinger reste prod en parallèle (bascule plus tard).
...
2. Une fois le build vert → URL `*.pages.dev` générée (ex. `neopro-exg.pages.dev`).
3. Vérifier :
   ```bash
   curl -I https://neopro-exg.pages.dev/
   curl -I https://neopro-exg.pages.dev/sites/ci-probe   # doit être 200 (SPA fallback)
   ```
...
> **Skip pour staging Option A.** On reste sur l'URL Cloudflare par défaut `https://neopro-exg.pages.dev`. Pour ajouter un custom domain `neopro-staging.kalonpartners.bzh` plus tard, il faudra d'abord migrer la zone `kalonpartners.bzh` chez Cloudflare (ADR-071 phase 1) — à planifier J5+ ou avec Gabin.
...
2. Désormais chaque PR déclenche un build + URL preview unique (ex. `pr-123.neopro-exg.pages.dev`).
...
curl -I https://neopro-exg.pages.dev/                       # 200, header server: cloudflare
curl -I https://neopro-exg.pages.dev/sites/ci-probe         # 200 (SPA fallback _redirects)
curl -sI https://neopro-exg.pages.dev/ | grep -i x-frame    # SAMEORIGIN

# Le dashboard tape bien l'API staging
# Ouvrir https://neopro-exg.pages.dev dans un navigateur
...
- [ ] `neopro-exg.pages.dev` actif (TLS Cloudflare, server: cloudflare)
...
**Livrable** : `neopro-exg.pages.dev` opérationnel, branché sur l'API staging J1. Prêt pour J3 (anonymized DB dump → tu auras enfin du contenu réel pour démo).

## Rollback

Tout est isolé. Pour annuler :

- Supprimer le projet Cloudflare Pages → DNS `neopro-exg.pages.dev` casse, prod intacte.
