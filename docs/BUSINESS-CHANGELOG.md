...
- **Deploy dashboard CI : 11min → <2min** ([#668](https://github.com/Tallec7/neopro/pull/668)) — la PR #625 avait appliqué les optims lftp aux DEUX jobs FTP (SaaS + Dashboard) en pensant régler un problème SaaS-spécifique. Les optims aident le SaaS (gros volume) mais pénalisent le dashboard (petit payload Angular) : `--use-pget-n=4` est dead-code en upload, `sync-mode off` + `parallel=5` déclenche le throttle Hostinger sur petits transferts. Revert ciblé du job dashboard uniquement vers la config pré-#625 (parallel=2, sync-mode default, pas de pget). Le job SaaS reste optimisé. Chaque release retombera à <2min sur le dashboard au lieu de 11min.
