...
- **Diffusion par côté pas encore rendue** : le moteur de **pliage par côté** (#1089) + la **composition N vidéos** (#1091) sont prouvés en labo, mais **le câblage contenu→rendu n'est pas fait**. Le contenu par côté se choisira sur la **variante led-perimeter d'une vidéo** (révision ADR-135), pas dans les Réglages (le `side_zones`/sélecteur de #1088 est supersédé, rework en cours). Tant que ce n'est pas livré, le mode `uniform` reste le rendu effectif. V1 = 1 vidéo par côté ; rotation multi-sponsors par côté = v2.
