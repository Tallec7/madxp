...
> **ADR liés** : [ADR-134](../../adr/ADR-134-led-perimeter-render-directly-folded.md) (studio rend directement plié), [ADR-135](../../adr/ADR-135-led-perimeter-per-side-zones.md) (pliage par côté + zones de contenu — **conçu, pas encore implémenté**), [ADR-128](../../adr/ADR-128-templates-studio-asset-directory.md) (bornes RAM render Remotion), [ADR-137](../../adr/ADR-137-display-geometry-owns-resolution.md) (l'écran porte sa géométrie ; résolution dérivée, `zones` retiré), [ADR-138](../../adr/ADR-138-led-canvas-derives-from-site-only.md) (canvas dérivé du terrain seul, pliage toujours par côté)
...
- **Diffusion par côté pas encore rendue** : le moteur de **pliage par côté** (#1089) + la **composition N vidéos** (#1091) sont prouvés en labo, mais **le câblage contenu→rendu n'est pas fait**. Le contenu par côté se choisira sur la **variante led-perimeter d'une vidéo** (révision ADR-135), pas dans les Réglages (le `side_zones`/sélecteur de #1088 a été **retiré** ; reste à câbler côté variante). Tant que ce n'est pas livré, le mode `uniform` reste le rendu effectif. V1 = 1 vidéo par côté ; rotation multi-sponsors par côté = v2.
