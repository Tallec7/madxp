...
- **Le contenu ne traverse jamais un angle** : chaque côté est une zone naturelle. `zones: 'uniform'` (même contenu partout : ruban continu Σ côtés, ADR-134) ou `'per-side'` (contenu + cadence par côté — [ADR-135](../../adr/ADR-135-led-perimeter-per-side-zones.md)). **État** : la **saisie** par côté est livrée (`side_zones[]` validé Joi + dashboard : 1 vidéo + cadence par côté, persistés en JSONB, alignés par index sur `sides`) ; la **diffusion** par côté (pliage par côté + runtime) suit en PRs incrémentales. Le panneau affiche un bandeau « diffusion par côté à venir » pour ne pas laisser croire que c'est déjà rendu.
...
- **Diffusion par côté pas encore rendue** : le **contenu + cadence par côté** ([ADR-135](../../adr/ADR-135-led-perimeter-per-side-zones.md)) est **saisissable et persisté** (étape 1 : `side_zones[]` + UI) mais **pas encore diffusé** — le pliage par côté (moteur) + le runtime (studio/Pi) suivent en PRs incrémentales. Tant que ce n'est pas livré, le mode `uniform` reste le rendu effectif (un ruban continu Σ côtés). V1 = 1 vidéo par côté ; rotation multi-sponsors par côté = v2.
