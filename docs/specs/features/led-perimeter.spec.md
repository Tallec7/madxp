...
> **ADR liés** : [ADR-134](../../adr/ADR-134-led-perimeter-render-directly-folded.md) (studio rend directement plié), [ADR-135](../../adr/ADR-135-led-perimeter-per-side-zones.md) (pliage par côté + zones de contenu — **conçu, pas encore implémenté**), [ADR-128](../../adr/ADR-128-templates-studio-asset-directory.md) (bornes RAM render Remotion), [ADR-137](../../adr/ADR-137-display-geometry-owns-resolution.md) (l'écran porte sa géométrie ; résolution dérivée, `zones` retiré), [ADR-138](../../adr/ADR-138-led-canvas-derives-from-site-only.md) (canvas dérivé du terrain seul, pliage toujours par côté), [ADR-139](../../adr/ADR-139-led-serve-folded-canvas.md) (le canvas plié est servi derrière `serve_folded`, éteint par défaut)
...
- **Le canvas d'un site se dérive du TERRAIN seul** (ADR-138) : `computeSiteCanvas()` est le point d'entrée unique et plie **toujours par côté**. Le contenu ne choisit que les sources (un fichier par côté, ou le même partout) — « uniforme » n'est plus une géométrie concurrente. Le contenu ne traverse donc plus les angles et le motif redémarre à chaque côté. Un `band_count` figé par un installateur qui ne correspond plus au dérivé est **signalé** (`confirmedIsStale`), jamais écrasé.
...
- **Le système PROPOSE le cadrage, l'opérateur tranche** : à l'upload d'une variante led-perimeter, `classifyVideoForRibbon()` classe la vidéo mesurée contre la géométrie du ruban et renvoie `fit_recommendation` — cadrage (`one-side` / `full-ribbon`), mise en page à pré-sélectionner, phrase en clair, et avertissements. Deux règles portent tout : une **largeur exacte prime sur le ratio** (une agence qui livre 1600 px pour un côté de 1600 px l'a fait exprès, même si son ratio s'écarte de 33 %), et le choix du pavage se décide sur le **taux de remplissage après mise à l'échelle**, jamais sur la largeur brute (un 1920×1080 est plus large qu'un côté mais n'en remplit que 18 % une fois ramené à la hauteur du ruban). Jamais bloquant : pas de conseil quand on ne sait pas.
- **Le canvas plié n'est servi que si le site l'a explicitement demandé** (ADR-139) : `displays[].led.canvas_in.serve_folded`, **sans défaut**. Allumé, la diffusion sert le canvas plié à la place du fichier brut ; éteint — partout aujourd'hui — rien ne change. `canvas_in.mode` ne pouvait pas jouer ce rôle : il vaut `'B'` par défaut sur tout le parc sans que personne l'ait choisi. L'activation se fait club par club, après avoir observé le montage réel (`npm run led:mire`).
- **Le pliage dégrade, il ne casse pas** : canvas pas encore fabriqué, cache injoignable ou profil illisible → le fichier brut est servi et la fabrication mise en file. Un déploiement ne doit jamais échouer à cause du pliage.
- **Le cache de canvas est clé par géométrie** : l'empreinte couvre côtés + pitch + hauteur + largeur de bande + ordre + source + cadrage. Changer la hauteur d'un ruban rend inatteignables tous les canvas de l'ancienne — l'invalidation est structurelle, il n'y a ni TTL ni purge.
...
- Uploader la vidéo sponsor réelle d'un club (1600×120) sur le ruban de Piraths (4 × 10 m, P6.25, 160 rangées) renvoie : cadrage « un côté », mise en page « Centré », et l'explication « Ta vidéo (1600×120) sera adaptée à un côté (1600×160), avec 20 px noirs en haut et en bas. Elle apparaîtra à l'identique sur les 4 côtés. » — plus l'avertissement « Étalé déformerait l'image de 33 % en hauteur ».
...
- **Pas encore actif en production** : le câblage contenu→rendu existe depuis ADR-139, mais **aucun des deux sites LED n'a `serve_folded` activé**. Le préalable n'est pas du code, c'est une observation : passer la mire sur un club installé pour lire le contrat d'entrée réel de son processeur. Tant que ce n'est pas fait, le fichier brut reste servi. V1 = 1 vidéo par côté ; rotation multi-sponsors par côté = v2.
