...
- **Cadrage de la conversion image → vidéo** : la sortie est toujours 1280x720. **Le visuel source est intégralement visible, quel que soit son ratio** — il n'est jamais rogné ni zoomé. Deux habillages du vide autour :
  - _bandes noires_ (défaut du formulaire Contenu) : `scale=1280:720:force_original_aspect_ratio=decrease` + `pad`.
  - _fond flou_ (défaut de la zone de drag & drop) : le **fond** déborde volontairement puis est recadré (`increase` + `crop` + `boxblur`), le **foreground** est contenu (`decrease`).
  - Invariant : le foreground doit être borné sur **les deux** dimensions (`scale=1280:720:…`). Une largeur libre (`scale=-1:720`) ne contraint que la hauteur — une bannière 1200x150 devient 5760x720 et l'overlay n'en garde que la bande centrale (incident 2026-08-04, visuel sponsor rendu en zoom avec le texte coupé).
