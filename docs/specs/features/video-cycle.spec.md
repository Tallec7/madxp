...
- **Cadrage de la conversion image → vidéo** : **le visuel source est intégralement visible et conserve son format** — jamais rogné, jamais zoomé, jamais déformé. Deux régimes de sortie :
  - _défaut_ : **ratio source préservé**. `scale='min(1920,iw)':'min(1920,ih)':force_original_aspect_ratio=decrease` (plafonne le plus grand côté à 1920 sans jamais agrandir), puis `scale=trunc(iw/2)*2:trunc(ih/2)*2` (dimensions paires, exigées par yuv420p/H.264 — une source 1201x151 casserait l'encodage), puis `setsar=1`. Un bandeau 1200x150 sort en 1200x150.
  - _habillage 16:9_ (option « Habiller en 16:9 (fond flou) », décochée par défaut) : sortie forcée en 1280x720, le vide autour du visuel étant rempli par une version floutée. Le **fond** déborde volontairement puis est recadré (`increase` + `crop` + `boxblur`), le **foreground** est contenu (`decrease`).
  - **Ne pas cuire de canvas fixe par défaut** : le player TV applique déjà `object-fit: contain` (`raspberry/.../tv.component.scss`). Padder à l'encodage cadre deux fois et gâche la définition du visuel utile (signalé 2026-08-04 : bandeau sponsor réduit à une bande fine au milieu d'un 1280x720).
  - Invariant du mode 16:9 : le foreground doit être borné sur **les deux** dimensions (`scale=1280:720:…`). Une largeur libre (`scale=-1:720`) ne contraint que la hauteur — une bannière 1200x150 devient 5760x720 et l'overlay n'en garde que la bande centrale (incident 2026-08-04, visuel sponsor rendu en zoom avec le texte coupé).
