...
> **Définition** : somme des durées (heures) de toutes les sessions match qui se sont déroulées sur la flotte MadXP pendant la semaine.
...
- **Reflète la valeur livrée au club** : 1h de match diffusé = 1h où MadXP crée de la valeur visible en tribune
...
Métrique de **rétention infrastructurelle** : un site qui n'utilise pas MadXP pendant 2 semaines consécutives est probablement perdu, même s'il paye encore son abonnement.
...
C'est la métrique **monétisable** côté annonceurs. Plus d'impressions = pricing power supérieur pour le club ET pour MadXP (si revenue share côté annonceur national/régie).

Cf. `BENCHMARK-COMPETITORS.md` : différenciateur fort MadXP vs Bodet/Stramatel = "Inventaire publicitaire centralisé multi-clubs".
...
| **Dashboard MadXP `/admin/metrics`** (à créer) | NSM + 4 inputs visibles en 1 page pour PM   | 🔮 NEXT (mois 2-3)                        |
...
- [ ] Confirmer modèle annonceurs (commission OUI/NON) — impacte si l'input metric B est aussi un revenu direct MadXP
