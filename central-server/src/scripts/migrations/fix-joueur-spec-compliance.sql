...
-- =============================================================================
UPDATE madxp_templates SET fps = 25
...
WHERE template_id = (SELECT id FROM madxp_templates WHERE name = 'BUT Simple')
  AND name = 'BUT_simple_B' AND duration_ms != 2760;

UPDATE template_layers SET duration_ms = 1920
WHERE template_id = (SELECT id FROM madxp_templates WHERE name = 'Joueur détaillé')
...
WHERE l.name = expected.n
  AND l.template_id = (SELECT id FROM madxp_templates WHERE name = 'Joueur détaillé');
...
UPDATE template_text_fields SET font_family = 'Bulevar', font_size = 389, position_y = 0.40, animation = 'none', always_visible = true,
  layer_id = (SELECT id FROM template_layers WHERE template_id = (SELECT id FROM madxp_templates WHERE name = 'BUT Simple') AND name = 'BUT_simple_C')
WHERE template_id = (SELECT id FROM madxp_templates WHERE name = 'BUT Simple')
  AND slot_key = 'prenom';
...
UPDATE template_image_slots SET
  ...
  layer_id = (SELECT id FROM template_layers WHERE template_id = (SELECT id FROM madxp_templates WHERE name = 'BUT Simple') AND name = 'A — intro hexagone (logo/numéro)')
WHERE template_id = (SELECT id FROM madxp_templates WHERE name = 'BUT Simple')
  AND slot_key = 'logoSrc';
...
FROM madxp_templates WHERE name = 'Joueur détaillé'
ON CONFLICT (template_id, key) DO NOTHING;

INSERT INTO template_options (template_id, key, label, type, values, default_value, user_editable, sort_order)
SELECT id, 'intro_mode', 'Intro', 'enum', '["logo","numero"]'::jsonb, 'logo', true, 1
FROM madxp_templates WHERE name = 'BUT Simple'
ON CONFLICT (template_id, key) DO NOTHING;
