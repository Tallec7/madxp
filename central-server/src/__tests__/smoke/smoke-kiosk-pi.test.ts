...
describe('Kiosk xdpyinfo dependency must be provisioned', () => {
  ...
  it('prepare-image first-boot-setup must NOT write WiFi secrets into club-config.json (ADR-074)', () => {
    // ADR-074 : le PSK hotspot est désormais cloud-canonical (sites.wifi_psk_encrypted).
    // club-config.json ne contient plus de secret → chmod 600 obsolète,
    // et l'écriture de wifiSSID/wifiPassword doit avoir disparu.
    const prepareImage = fs.readFileSync(
      path.join(repoRoot, 'raspberry/tools/prepare-image.sh'),
      'utf8'
    );
    expect({
      noWifiPassword: !/["']wifiPassword["']\s*:/.test(prepareImage),
      noWifiSsid: !/["']wifiSSID["']\s*:/.test(prepareImage),
