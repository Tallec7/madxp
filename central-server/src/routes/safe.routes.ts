...
router.get('/proposals/:id', authenticate, requireRole('admin'), validateParams(paramSchemas.id), safeController.getProposal);
...
router.put('/proposals/:id', authenticate, requireRole('admin'), validateParams(paramSchemas.id), validate(schemas.updateProposalStatus), safeController.updateProposalStatus);

// Suppression d'une proposal
router.delete('/proposals/:id', authenticate, requireRole('admin'), validateParams(paramSchemas.id), safeController.deleteProposal);

// Mise à jour d'un epic (nom et/ou statut) — write-back dans FEATURES.md
router.put('/epics/:id', authenticate, requireRole('admin'), validateParams(paramSchemas.id), validate(schemas.updateEpic), safeController.updateEpic);
...
router.put('/risks/:id/roam-status', authenticate, requireRole('admin'), validateParams(paramSchemas.id), validate(schemas.updateRiskRoamStatus), safeController.updateRiskRoamStatus);

// Mise à jour du contenu d'une proposal (titre + markdown)
router.put('/proposals/:id/content', authenticate, requireRole('admin'), validateParams(paramSchemas.id), validate(schemas.updateProposalContent), safeController.updateProposalContent);
