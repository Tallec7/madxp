...
export async function generateAdvertiserReport(
  ...
  try {
    ...
    // Métriques globales
    // Note: Les colonnes completed, video_duration, audience_estimate existent en live
    // (ajoutées par la migration rename-sponsor-to-advertiser)
...
// ============================================================================
// RAPPORT SITE SPONSOR — 1 PAGE COMMERCIAL (Palier 2)
// ============================================================================

interface SiteSponsorReportData {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      data.byEventType.forEach(evt => {
        const label = eventTypeLabels[evt.eventType] || evt.eventType;
        ...
        doc
          ...
          .fillColor(COLORS.text)
          .font('Helvetica')
          .text(`${label}: ${pct}% (${evt.count})`, barWidth + 50, y + 3);
