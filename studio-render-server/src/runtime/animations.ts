...
export function computeAnimation(
  ...
  switch (preset) {
    ...
    case 'zoom': {
      ...
      // SPEC v1 — interpolation linéaire sur `appearDuration` (au lieu de
      // spring) pour que la durée soit respectée à la lettre. Le spring
      // convergait en ~0.5s indépendamment de appearDuration.
      const p = progress(params);
      const pres = presence(p, dir);
      return {
        opacity: interpolate(pres, [0, 1], [0, 1]),
    ...
    case 'scale-only': {
      // SPEC v1 — scale anim linéaire sans fade-in. Opacity = 1 dès appearAt.
      const from = params.scaleFrom ?? 0.5;
      const to = params.scaleTo ?? 1.0;
      const p = progress(params);
      const pres = presence(p, dir);
      return {
