...
export function computeImageStyle(input: FitModeInput): FitModeResult {
  ...
  switch (fitMode) {
    ...
    case 'fill-width-anchor-top':
      // SPEC v1 — width:100% + height:auto laisse le browser scaler l'image en
      // préservant le ratio (jusqu'à matcher la largeur du wrapper). Sans
      // object-fit qui figeait la taille native. Si la photo source est plus
      // haute que le wrapper, ça déborde en bas (overflow: 'bottom').
      return {
        wrapper: wrapperBase,
        img: {
          width: '100%',
